 ...
 */
const { computeHealthScore, tierForScore } = require("../lib/plantHealth");

describe("plantHealth — Iter-48 diligence-tuned scale", () => {
  test("no findings → perfect 100 / Excellent", () => {
    const r = computeHealthScore({ high: 0, medium: 0, low: 0 });
    expect(r.score).toBe(100);
    expect(tierForScore(r.score).key).toBe("excellent");
  });
  ...
  test("catastrophic (10 high, 10 medium, 10 low) → clamped to 0 / Critical", () => {
    ...
    expect(tierForScore(r.score).key).toBe("critical");
  });
  ...
  test("tier boundaries partition [0..100] cleanly", () => {
    // Every integer score should map to exactly one tier.
    const seen = new Set();
    for (let s = 0; s <= 100; s++) {
